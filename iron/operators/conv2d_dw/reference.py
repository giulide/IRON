...
def generate_golden_reference(
    c: int, h_in: int, w_in: int, k_h: int, k_w: int,
    padding=0, dtype="bf16", seed=42
):
    ...
    #   input:  (1, C, H, W)  — batch=1, C channels
    #   weight: (C, 1, kH, kW) — one kernel per channel
    #   output: (1, C, H_out, W_out)
    ...
    # Depthwise convolution via groups=c

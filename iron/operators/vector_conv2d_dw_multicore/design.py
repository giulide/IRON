...
def conv2d_dw_multicore(dev, h, w, c, k_h, k_w, padding, num_cores):
    ...
    L1-sized 2-D grid of sub-tiles (H and W tiled) over time (double-buffered).
    All of a core's sub-tiles are moved with ONE strided DMA (a 4-D "repeat"
    over the sub-tile grid), so the number of DMA descriptors stays tiny no
    matter how large the image is. The input is host-pre-padded (see tiling.py),
    so the kernel is a pure conv (W = the sub-tile width) with no boundary
    handling and every sub-tile sits at a regular offset.
    """
    ...
    def make_body():
        def body(of_in_, of_w_, of_out_, kernel):
            ...
            for _ in range(num_tiles):  # Python-unrolled (constant)
    ...
    # outputs. Sub-tiles overlap by (k-1) halo (stride h_out_tile < span h_in_tile,
    # stride w_out_tile < span w_in_tile).
    rt = Runtime()

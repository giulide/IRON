...
padded buffer shapes. Parallelization for one AIE column:
  ...
  * Each core streams its band as L1-sized SUB-TILES (a 2-D H x W grid) over
    time (double-buffered DMA <-> compute); wide images are tiled along W too,
    so a single row that would overflow L1 is handled.
  * Every core's transfers are issued as ONE strided DMA (a "repeat" over its
    sub-tile grid) => a constant, tiny number of DMA descriptors.
...
def compute_tiling(h, w, c, k_h, k_w, padding, num_cores):
    ...
    # --- Width tiling: keep full width if a k_h-row band fits, else tile W ---
